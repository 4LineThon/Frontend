...
const CreateCalendar = ({ groupTimetableData, userid, onChange = () => {} }) => {
  ...
  const generateTimeSlots = (start, end) => {
    ...
    while (current < endTime) { // 끝 시간을 포함하지 않음
...
{timeSlots.length > 0 && gridState.length > 0 && 
  timeSlots.map((_, timeIndex) =>
    groupTimetableData.map((_, dayIndex) => (
      <rect
        ...
        width="36"
        height="18"
        fill={gridState[timeIndex]?.[dayIndex] ? "#423E59" : "#D9D9D9"} // ?. 사용
        fillOpacity={gridState[timeIndex]?.[dayIndex] ? 0.8 : 1} // ?. 사용
        stroke="#423E59"
        strokeWidth="1"
